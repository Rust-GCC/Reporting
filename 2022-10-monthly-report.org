...
1. How do we deal with generic intrinsics
2. Example of `copy_nonoverlapping` which is quite small and easy to understand

**** data prefetch intrinsics

1. Only wants constants! But rustc allows anything! But LLVM really doesn't! Playground links
2. Solutions: constant folding trial? switch/match statement?

**** unsigned atomic intrinsics

1. Again, generic intrinsics from libcore
2. But GCC's atomic intrinsics only accept unsigned values! Unless I'm doing it wrong :)
