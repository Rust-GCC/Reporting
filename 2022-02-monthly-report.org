...
1/ Either a singular identifier, with zero or one commas (pattern: `,`, kleene operator: `?` (0 -> 1))
...
In order to implement some specific behavior, the rust standard library requires some macros to be built into the compiler. You can find a full list [here](https://github.com/Rust-GCC/gccrs/issues/927).
